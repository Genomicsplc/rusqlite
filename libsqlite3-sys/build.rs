...
mod bindings {
    ...
    pub fn write_to_out_dir(header: HeaderLocation, out_path: &Path) {
        ...
        #[cfg(feature = "loadable_extension")]
        {
            ...
            #[cfg(not(feature = "variadics"))]
            {
                output_string.push_str(
                    r#"

// support for variadics is not enabled, so define __va_list_tag as an alias for *void
type __va_list_tag = *mut ::std::os::raw::c_void;
...
"#,
	...
	let mut wrapper_fn_inputs_map: BTreeMap<String, syn::punctuated::Punctuated<syn::BareFnArg, syn::token::Comma>> = BTreeMap::new();

	// always generate a wrapper function of the same name as the api function name with no variadic arguments
	wrapper_fn_inputs_map.insert(api_fn_name.to_string(), generate_varargs_input_idents(field_ident, &bare_fn, &vec![]));
        ...
        if bare_fn.variadic.is_some() {
            ...
            let mut_void_type: syn::Type = syn::parse2(quote!(*mut ::core::ffi::c_void))
                .expect("failed to parse c_char type");
            let c_int_type: syn::Type = syn::parse2(quote!(::std::os::raw::c_int))
                .expect("failed to parse c_int type");
	    ...
	    // functions we don't handle will have 
            match api_fn_name {
                "sqlite3_db_config" => {
                    ...
                    wrapper_fn_inputs_map.insert("sqlite3_db_config_constchar".to_string(), generate_varargs_input_idents(field_ident, &bare_fn, &vec![const_c_char_type.clone()])); // used for SQLITE_DBCONFIG_MAINDBNAME
                    wrapper_fn_inputs_map.insert("sqlite3_db_config_void_int_mutint".to_string(), generate_varargs_input_idents(field_ident, &bare_fn, &vec![mut_void_type.clone(), c_int_type.clone(), mut_c_int_type.clone()])); // used for SQLITE_DBCONFIG_LOOKASIDE
                    wrapper_fn_inputs_map.insert("sqlite3_db_config_int_mutint".to_string(), generate_varargs_input_idents(field_ident, &bare_fn, &vec![c_int_type.clone(), mut_c_int_type.clone()])); // used for all other configuration verbs
                }
                "sqlite3_vtab_config" => {
                    // https://sqlite.org/c3ref/c_vtab_constraint_support.html
		    wrapper_fn_inputs_map.insert("sqlite3_vtab_config_int".to_string(), generate_varargs_input_idents(field_ident, &bare_fn, &vec![c_int_type.clone()])); // used for SQLITE_VTAB_CONSTRAINT_SUPPORT
		}
                _ => {},
            };
        }
